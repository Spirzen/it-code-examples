/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-03-java
 ...
 *
 * Usage:
 *   node scripts/migrate-java-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-03-java');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'java-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-03-java';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `java-503-${art}`;
  const seriesTitle = meta.title ?? `Java ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `java-503-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB java dir not found: ${KB_DIR}`);
  }

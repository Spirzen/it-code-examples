/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-10-typescript
 ...
 *
 * Usage:
 *   node scripts/migrate-typescript-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-10-typescript');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'typescript-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-10-typescript';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `typescript-510-${art}`;
  const seriesTitle = meta.title ?? `TypeScript ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `typescript-510-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['typescript', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB typescript dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает листинги из docs/encyclopedia/4-code-dev/4-09-zavisimosti
 ...
 *
 * Usage:
 *   node scripts/migrate-dependencies-examples-from-kb.mjs [--dry-run] [--min-lines=8] [--only=1.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'dependencies-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-09-zavisimosti';
...
function processArticle(filePath) {
  ...
  const seriesId = `code-409-${art}`;
  const seriesTitle = meta.title ?? `Зависимости ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-409-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['dependencies', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB dependencies dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/3-data-markup/3-06-nosql
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Usage:
 *   node scripts/migrate-nosql-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=41.md]
 */
...
const KB_NOSQL = path.join(KB_ROOT, 'docs', 'encyclopedia', '3-data-markup', '3-06-nosql');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'nosql-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/3-data-markup/3-06-nosql';
...
function processArticle(filePath) {
  ...
  const seriesId = `nosql-${art}`;
  const seriesTitle = meta.title ?? `NoSQL ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `nosql-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_NOSQL)) {
    throw new Error(`KB nosql dir not found: ${KB_NOSQL}`);
  }

  const files = fs
    .readdirSync(KB_NOSQL)
  ...
  for (const file of files) {
    const full = path.join(KB_NOSQL, file);

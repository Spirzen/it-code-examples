/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-05-csharp
 ...
 *
 * Usage:
 *   node scripts/migrate-csharp-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-05-csharp');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'csharp-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-05-csharp';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `csharp-505-${art}`;
  const seriesTitle = meta.title ?? `C# ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `csharp-505-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB csharp dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает листинги из docs/encyclopedia/4-code-dev/4-08-oop
 ...
 *
 * Usage:
 *   node scripts/migrate-oop-examples-from-kb.mjs [--dry-run] [--min-lines=8] [--only=1.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'oop-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-08-oop';
...
const SKIP_FILES = new Set(['intro.md', '98.md', '99.md']);
...
function processArticle(filePath) {
  ...
  const seriesId = `code-408-${art}`;
  const seriesTitle = meta.title ?? `ООП ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-408-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB OOP dir not found: ${KB_DIR}`);
  }

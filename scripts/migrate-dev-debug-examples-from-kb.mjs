/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/4-code-dev/4-14-razrabotka-i-otladka
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Короткие bash-команды, mermaid и text остаются в статье (min-lines по умолчанию 15).
 *
 * Usage:
 *   node scripts/migrate-dev-debug-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=117.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-14-razrabotka-i-otladka',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'dev-debug-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-14-razrabotka-i-otladka';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `code-414-${art}`;
  const seriesTitle = meta.title ?? `Разработка и отладка ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-414-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['dev-debug', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB dev-debug dir not found: ${KB_DIR}`);
  }

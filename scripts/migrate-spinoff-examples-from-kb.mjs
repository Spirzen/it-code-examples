/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/9-spinoff
 ...
 * Интерактив (ExternalPlayEmbed) не трогаем — он уже в it-play.
 *
 * Usage:
 *   node scripts/migrate-spinoff-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=6.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '9-spinoff');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'spinoff-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/9-spinoff';
...
const SKIP_FILES = new Set(['intro.md', '998.md', '999.md', '1001.md', 'spinoff.md']);
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `sp-9-${art}`;
  const seriesTitle = meta.title ?? `Spinoff ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `sp-9-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function matchesOnly(relativePath) {
  ...
  return (
    ...
    relativePath.endsWith(`/${normalized}.md`) ||
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB spinoff dir not found: ${KB_DIR}`);
  }

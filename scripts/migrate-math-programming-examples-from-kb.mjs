/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/3-data-markup/3-12-matematicheskoe-programmirovanie
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Usage:
 *   node scripts/migrate-math-programming-examples-from-kb.mjs [--dry-run] [--min-lines=12] [--only=9.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '3-data-markup',
  '3-12-matematicheskoe-programmirovanie',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'math-programming-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/3-data-markup/3-12-matematicheskoe-programmirovanie';

const args = process.argv.slice(2);
const DRY_RUN = args.includes('--dry-run');
const ONLY = args.find((a) => a.startsWith('--only='))?.slice('--only='.length);
const MIN_LINES = Number(args.find((a) => a.startsWith('--min-lines='))?.slice('--min-lines='.length) ?? 12);
...
function processArticle(filePath) {
  ...
  const seriesId = `mp-${art}`;
  const seriesTitle = meta.title ?? `Математическое программирование ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `mp-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['math-programming', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB math-programming dir not found: ${KB_DIR}`);
  }

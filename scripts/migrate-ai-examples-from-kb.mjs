/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/6-ai
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Обрабатывает все подразделы 6-ai (вложенные папки). Короткие фрагменты, mermaid и text
 * остаются в статье (min-lines по умолчанию 15).
 *
 * Usage:
 *   node scripts/migrate-ai-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=6-02-mashinnoe-obuchenie/1.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '6-ai');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'ai-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/6-ai';
...
const SKIP_FILES = new Set(['intro.md', '998.md', '999.md', '1001.md', 'ai.md']);

const SKIP_FENCE_LANGS = new Set(['text', 'plain', 'plaintext', 'mermaid', 'scratch', 'regex', 'txt', 'markdown', 'prompt', 'env', '']);
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `ai-6-${art}`;
  const seriesTitle = meta.title ?? `ИИ ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `ai-6-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB ai dir not found: ${KB_DIR}`);
  }

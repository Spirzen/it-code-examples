/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-02-python
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Обрабатывает вложенную папку kivy-praktikum/. Короткие фрагменты, mermaid и text
 * остаются в статье (min-lines по умолчанию 15).
 *
 * Usage:
 *   node scripts/migrate-python-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-02-python');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'python-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-02-python';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `py-502-${art}`;
  const seriesTitle = meta.title ?? `Python ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `py-502-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB python dir not found: ${KB_DIR}`);
  }

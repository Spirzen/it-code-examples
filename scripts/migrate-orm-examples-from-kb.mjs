/**
 * Извлекает листинги из docs/encyclopedia/4-code-dev/4-10-orm-i-rabota-s-dannymi
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Русский псевдокод (text, plain, без языка), mermaid и короткие фрагменты остаются в статье.
 *
 * Usage:
 *   node scripts/migrate-orm-examples-from-kb.mjs [--dry-run] [--min-lines=8] [--only=1.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-10-orm-i-rabota-s-dannymi',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'orm-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-10-orm-i-rabota-s-dannymi';
...
function mapLang(fenceLang) {
  ...
  return FENCE_TO_FOLDER[key] ?? null;
}
...
function processArticle(filePath) {
  ...
  const seriesId = `code-410-${art}`;
  const seriesTitle = meta.title ?? `ORM ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-410-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB ORM dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-07-php
 ...
 *
 * Usage:
 *   node scripts/migrate-php-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-07-php');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'php-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-07-php';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `php-507-${art}`;
  const seriesTitle = meta.title ?? `PHP ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `php-507-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB php dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-27-1s
 ...
 *
 * Usage:
 *   node scripts/migrate-1s-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=113.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-27-1s');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, '1s-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-27-1s';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `bsl-527-${art}`;
  const seriesTitle = meta.title ?? `1С ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `bsl-527-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB 1s dir not found: ${KB_DIR}`);
  }

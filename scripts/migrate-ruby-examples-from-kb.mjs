/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-11-ruby
 ...
 *
 * Usage:
 *   node scripts/migrate-ruby-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-11-ruby');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'ruby-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-11-ruby';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `ruby-511-${art}`;
  const seriesTitle = meta.title ?? `Ruby ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `ruby-511-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB ruby dir not found: ${KB_DIR}`);
  }

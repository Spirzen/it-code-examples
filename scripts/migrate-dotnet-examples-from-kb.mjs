/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-04-platforma-dotnet
 ...
 *
 * Usage:
 *   node scripts/migrate-dotnet-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=17.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-04-platforma-dotnet');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'dotnet-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-04-platforma-dotnet';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `dotnet-504-${art}`;
  const seriesTitle = meta.title ?? `.NET ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `dotnet-504-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB dotnet dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-16-starye-yazyki/visual-basic
 ...
 *
 * Usage:
 *   node scripts/migrate-visual-basic-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=7.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'visual-basic-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-16-starye-yazyki/visual-basic';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `vb-516-${art}`;
  const seriesTitle = meta.title ?? `Visual Basic ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `vb-516-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['visual-basic', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB visual-basic dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/3-data-markup/3-10-css
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Usage:
 *   node scripts/migrate-css-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=113.md]
 */
...
const KB_CSS = path.join(KB_ROOT, 'docs', 'encyclopedia', '3-data-markup', '3-10-css');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'css-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/3-data-markup/3-10-css';
...
function processArticle(filePath) {
  ...
  const seriesId = `css-${art}`;
  const seriesTitle = meta.title ?? `CSS ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `css-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_CSS)) {
    throw new Error(`KB css dir not found: ${KB_CSS}`);
  }
  ...
  for (const file of files) {
    const full = path.join(KB_CSS, file);

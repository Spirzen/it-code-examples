/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-15-lua-i-luau
 ...
 *
 * Usage:
 *   node scripts/migrate-lua-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-15-lua-i-luau');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'lua-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-15-lua-i-luau';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `lua-515-${art}`;
  const seriesTitle = meta.title ?? `Lua ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `lua-515-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB lua dir not found: ${KB_DIR}`);
  }

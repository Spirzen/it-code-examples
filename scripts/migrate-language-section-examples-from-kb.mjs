/**
 * Извлекает тяжёлые листинги из разделов encyclopedia/5-languages/5-17…5-24
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Usage:
 *   node scripts/migrate-language-section-examples-from-kb.mjs <section> [--dry-run] [--min-lines=15] [--only=2.md]
 *
 * Sections: haskell, scala, elixir, zig, nim, dart, r, julia
 */
...
const SECTIONS = {
  haskell: {
    kbSubdir: '5-17-haskell',
    encyclopediaBase: '/encyclopedia/5-languages/5-17-haskell',
    seriesPrefix: 'haskell-517',
    tag: 'haskell',
    manifest: 'haskell-examples-manifest.json',
  },
  scala: {
    kbSubdir: '5-18-scala',
    encyclopediaBase: '/encyclopedia/5-languages/5-18-scala',
    seriesPrefix: 'scala-518',
    tag: 'scala',
    manifest: 'scala-examples-manifest.json',
  },
  elixir: {
    kbSubdir: '5-19-elixir',
    encyclopediaBase: '/encyclopedia/5-languages/5-19-elixir',
    seriesPrefix: 'elixir-519',
    tag: 'elixir',
    manifest: 'elixir-examples-manifest.json',
  },
  zig: {
    kbSubdir: '5-20-zig',
    encyclopediaBase: '/encyclopedia/5-languages/5-20-zig',
    seriesPrefix: 'zig-520',
    tag: 'zig',
    manifest: 'zig-examples-manifest.json',
  },
  nim: {
    kbSubdir: '5-21-nim',
    encyclopediaBase: '/encyclopedia/5-languages/5-21-nim',
    seriesPrefix: 'nim-521',
    tag: 'nim',
    manifest: 'nim-examples-manifest.json',
  },
  dart: {
    kbSubdir: '5-22-dart',
    encyclopediaBase: '/encyclopedia/5-languages/5-22-dart',
    seriesPrefix: 'dart-522',
    tag: 'dart',
    manifest: 'dart-examples-manifest.json',
  },
  r: {
    kbSubdir: '5-23-r',
    encyclopediaBase: '/encyclopedia/5-languages/5-23-r',
    seriesPrefix: 'r-523',
    tag: 'r',
    manifest: 'r-examples-manifest.json',
  },
  julia: {
    kbSubdir: '5-24-julia',
    encyclopediaBase: '/encyclopedia/5-languages/5-24-julia',
    seriesPrefix: 'julia-524',
    tag: 'julia',
    manifest: 'julia-examples-manifest.json',
  },
};

const args = process.argv.slice(2);
const SECTION_KEY = args.find((a) => !a.startsWith('--'));
...
function buildDocUrl(encyclopediaBase, relativePath) {
  const slug = relativePath.replace(/\.md$/, '').replace(/\\/g, '/');
  return `https://spirzen.ru${encyclopediaBase}/${slug}`;
}
...
function processArticle(filePath, relativePath, sectionConfig) {
  ...
  const docUrl = buildDocUrl(sectionConfig.encyclopediaBase, relativePath);
  const seriesId = `${sectionConfig.seriesPrefix}-${art}`;
  const seriesTitle = meta.title ?? `${sectionConfig.tag} ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `${sectionConfig.seriesPrefix}-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: [sectionConfig.tag, 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  const sectionConfig = SECTIONS[SECTION_KEY];
  if (!sectionConfig) {
    console.error(`Unknown section: ${SECTION_KEY ?? '(none)'}`);
    console.error(`Available: ${Object.keys(SECTIONS).join(', ')}`);
    process.exit(1);
  }

  const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', sectionConfig.kbSubdir);
  const MANIFEST_PATH = path.join(__dirname, sectionConfig.manifest);

  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB dir not found: ${KB_DIR}`);
  }
  ...
  const manifest = {
    ...
    minLines: MIN_LINES,
    section: SECTION_KEY,
    encyclopediaPath: sectionConfig.encyclopediaBase,
    articles: {},
  };
  ...
  for (const {full, relativePath} of files) {
    const entries = processArticle(full, relativePath, sectionConfig);
  ...
  console.log(
    `[${SECTION_KEY}] Done. ${totalExamples} example(s) from ${totalBlocks} block(s) across ${Object.keys(manifest.articles).length} article(s)${DRY_RUN ? ' (dry-run)' : ''}.`,
  );

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/4-code-dev/4-11-desktopnye-prilozheniya
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Обрабатывает вложенную папку wpf-praktikum/. Русский псевдокод, mermaid и короткие
 * фрагменты остаются в статье.
 *
 * Usage:
 *   node scripts/migrate-desktop-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=1192.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-11-desktopnye-prilozheniya',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'desktop-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-11-desktopnye-prilozheniya';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `code-411-${art}`;
  const seriesTitle = meta.title ?? `Десктоп ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-411-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB desktop dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-16-starye-yazyki/Pascal
 ...
 *
 * Usage:
 *   node scripts/migrate-pascal-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=7.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'pascal-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-16-starye-yazyki/Pascal';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `pascal-516-${art}`;
  const seriesTitle = meta.title ?? `Pascal ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `pascal-516-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB Pascal dir not found: ${KB_DIR}`);
  }

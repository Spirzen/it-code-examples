/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-14-swift
 ...
 *
 * Usage:
 *   node scripts/migrate-swift-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-14-swift');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'swift-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-14-swift';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `swift-514-${art}`;
  const seriesTitle = meta.title ?? `Swift ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `swift-514-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB swift dir not found: ${KB_DIR}`);
  }

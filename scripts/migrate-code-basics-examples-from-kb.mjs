/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Usage:
 *   node scripts/migrate-code-basics-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=4.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-02-chto-takoe-kod-i-kak-on-rabotaet',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'code-basics-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-02-chto-takoe-kod-i-kak-on-rabotaet';
...
const SKIP_FILES = new Set(['intro.md', '617.md', '998.md', '999.md']);
...
function processArticle(filePath) {
  ...
  const seriesId = `code-402-${art}`;
  const seriesTitle = meta.title ?? `Код ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-402-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['code-basics', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB code-basics dir not found: ${KB_DIR}`);
  }

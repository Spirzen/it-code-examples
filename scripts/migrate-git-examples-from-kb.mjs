/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git
 ...
 *
 * Usage:
 *   node scripts/migrate-git-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=116.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-13-osnovy-raboty-s-git',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'git-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-13-osnovy-raboty-s-git';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `code-413-${art}`;
  const seriesTitle = meta.title ?? `Git ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-413-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB git dir not found: ${KB_DIR}`);
  }

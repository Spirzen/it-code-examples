/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-26-powershell
 ...
 *
 * Usage:
 *   node scripts/migrate-powershell-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=117.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-26-powershell');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'powershell-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-26-powershell';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `powershell-526-${art}`;
  const seriesTitle = meta.title ?? `PowerShell ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `powershell-526-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['powershell', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB powershell dir not found: ${KB_DIR}`);
  }
  ...
  if (!DRY_RUN && totalExamples === 0) {
    console.log(`Manifest unchanged (no new examples; existing: ${MANIFEST_PATH}).`);
  }
}

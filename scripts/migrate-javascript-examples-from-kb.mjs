/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-01-javascript
 ...
 *
 * Usage:
 *   node scripts/migrate-javascript-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-01-javascript');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'javascript-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-01-javascript';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `js-501-${art}`;
  const seriesTitle = meta.title ?? `JavaScript ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `js-501-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['javascript', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB javascript dir not found: ${KB_DIR}`);
  }

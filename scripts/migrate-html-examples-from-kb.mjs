/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/3-data-markup/3-09-html
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Usage:
 *   node scripts/migrate-html-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_HTML = path.join(KB_ROOT, 'docs', 'encyclopedia', '3-data-markup', '3-09-html');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'html-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/3-data-markup/3-09-html';
...
function processArticle(filePath) {
  ...
  const seriesId = `html-${art}`;
  const seriesTitle = meta.title ?? `HTML ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `html-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_HTML)) {
    throw new Error(`KB html dir not found: ${KB_HTML}`);
  }
  ...
  for (const file of files) {
    const full = path.join(KB_HTML, file);

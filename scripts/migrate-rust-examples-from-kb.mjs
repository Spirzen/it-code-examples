/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-13-rust
 ...
 *
 * Usage:
 *   node scripts/migrate-rust-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-13-rust');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'rust-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-13-rust';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `rust-513-${art}`;
  const seriesTitle = meta.title ?? `Rust ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `rust-513-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB rust dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-08-smalltalk
 ...
 *
 * Usage:
 *   node scripts/migrate-smalltalk-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-08-smalltalk');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'smalltalk-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-08-smalltalk';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `smalltalk-508-${art}`;
  const seriesTitle = meta.title ?? `Smalltalk ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `smalltalk-508-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['smalltalk', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB smalltalk dir not found: ${KB_DIR}`);
  }

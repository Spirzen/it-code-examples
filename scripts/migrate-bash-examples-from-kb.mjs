/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-25-bash
 ...
 *
 * Usage:
 *   node scripts/migrate-bash-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=117.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-25-bash');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'bash-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-25-bash';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `bash-525-${art}`;
  const seriesTitle = meta.title ?? `Bash ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `bash-525-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB bash dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-09-kotlin
 ...
 *
 * Usage:
 *   node scripts/migrate-kotlin-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-09-kotlin');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'kotlin-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-09-kotlin';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `kotlin-509-${art}`;
  const seriesTitle = meta.title ?? `Kotlin ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `kotlin-509-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB kotlin dir not found: ${KB_DIR}`);
  }

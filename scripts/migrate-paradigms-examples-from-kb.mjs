/**
 * Извлекает листинги из docs/encyclopedia/4-code-dev/4-07-paradigmy-i-urovni-abstraktsii
 ...
 *
 * Usage:
 *   node scripts/migrate-paradigms-examples-from-kb.mjs [--dry-run] [--min-lines=8] [--only=113.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-07-paradigmy-i-urovni-abstraktsii',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'paradigms-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-07-paradigmy-i-urovni-abstraktsii';
...
function processArticle(filePath) {
  ...
  const seriesId = `code-407-${art}`;
  const seriesTitle = meta.title ?? `Парадигмы ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-407-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['paradigms', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB paradigms dir not found: ${KB_DIR}`);
  }

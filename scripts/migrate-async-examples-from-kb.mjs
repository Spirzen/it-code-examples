/**
 * Извлекает листинги из docs/encyclopedia/4-code-dev/4-05-asinhronnost
 ...
 *
 * Usage:
 *   node scripts/migrate-async-examples-from-kb.mjs [--dry-run] [--min-lines=8] [--only=11.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'async-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-05-asinhronnost';
...
function processArticle(filePath) {
  ...
  const seriesId = `code-405-${art}`;
  const seriesTitle = meta.title ?? `Асинхронность ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-405-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['async', 'concurrency', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB async dir not found: ${KB_DIR}`);
  }

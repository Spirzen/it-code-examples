/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-16-starye-yazyki/Fortran
 ...
 *
 * Usage:
 *   node scripts/migrate-fortran-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=7.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'fortran-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-16-starye-yazyki/Fortran';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `fortran-516-${art}`;
  const seriesTitle = meta.title ?? `Fortran ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `fortran-516-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB Fortran dir not found: ${KB_DIR}`);
  }

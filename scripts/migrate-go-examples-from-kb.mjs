/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-10-go
 ...
 *
 * Usage:
 *   node scripts/migrate-go-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-10-go');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'go-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-10-go';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `go-510-${art}`;
  const seriesTitle = meta.title ?? `Go ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `go-510-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB go dir not found: ${KB_DIR}`);
  }

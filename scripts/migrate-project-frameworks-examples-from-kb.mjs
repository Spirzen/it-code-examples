/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/4-code-dev/4-04-proekt-i-freymvorki
 ...
 *
 * Usage:
 *   node scripts/migrate-project-frameworks-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=2.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-04-proekt-i-freymvorki',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'project-frameworks-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-04-proekt-i-freymvorki';
...
function processArticle(filePath) {
  ...
  const seriesId = `code-404-${art}`;
  const seriesTitle = meta.title ?? `Проект и фреймворки ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-404-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['project-frameworks', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB project-frameworks dir not found: ${KB_DIR}`);
  }

 ...
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-12-groovy
 ...
 *   node scripts/migrate-groovy-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-12-groovy');

const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');

const MANIFEST_PATH = path.join(__dirname, 'groovy-examples-manifest.json');

const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-12-groovy';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `groovy-512-${art}`;

  const seriesTitle = meta.title ?? `Groovy ${art}`;
  ...
  for (const group of groups) {
    ...
    const slugName = `groovy-512-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  ...
  if (!fs.existsSync(KB_DIR)) {
    ...
    throw new Error(`KB groovy dir not found: ${KB_DIR}`);

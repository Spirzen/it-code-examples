/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-16-starye-yazyki/assembler
 ...
 *
 * Usage:
 *   node scripts/migrate-assembler-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=7.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'assembler-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-16-starye-yazyki/assembler';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `assembler-516-${art}`;
  const seriesTitle = meta.title ?? `Assembler ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `assembler-516-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['assembler', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB assembler dir not found: ${KB_DIR}`);
  }

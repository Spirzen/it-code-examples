/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-16-starye-yazyki/Lisp
 ...
 *
 * Usage:
 *   node scripts/migrate-lisp-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=811.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'lisp-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-16-starye-yazyki/Lisp';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `lisp-516-${art}`;
  const seriesTitle = meta.title ?? `Lisp ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `lisp-516-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB Lisp dir not found: ${KB_DIR}`);
  }

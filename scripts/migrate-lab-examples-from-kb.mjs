/**
 * Извлекает тяжёлые листинги из docs/lab/examples (it-knowledge-base) в examples/.
 * Группирует блоки кода внутри одной секции (##/###) в один пример с вкладками.
 *
 * Usage:
 *   node scripts/migrate-lab-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=1151.md]
 */
...
const KB_EXAMPLES = path.join(KB_ROOT, 'docs', 'lab', 'examples');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'lab-examples-manifest.json');
...
function buildExampleSlug(article, section, index) {
  const base = `lab-${article}-${section.anchor || slugify(section.title)}`;
  return index > 0 ? `${base}-${index + 1}` : base;
}

function buildDocUrl(meta, article) {
  if (meta.slug) {
    const slugPath = meta.slug.replace(/^\/lab\/Примеры\//, '/lab/Примеры/');
    return `https://spirzen.ru${slugPath.startsWith('/') ? slugPath : `/${slugPath}`}`;
  }
  return `https://spirzen.ru/lab/Примеры/${article}`;
}
...
function processArticle(filePath) {
  ...
  if (filename === 'intro.md') return null;
  ...
  const docUrl = buildDocUrl(meta, art);
  const seriesId = `lab-${art}`;
  const seriesTitle = meta.title ?? `Lab ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `lab-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_EXAMPLES)) {
    throw new Error(`KB examples dir not found: ${KB_EXAMPLES}`);
  }

  const files = fs
    .readdirSync(KB_EXAMPLES)
  ...
  for (const file of files) {
    const full = path.join(KB_EXAMPLES, file);

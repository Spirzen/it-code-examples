/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/4-code-dev/4-16-parallelnye-vychisleniya
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Псевдокод (text), mermaid и короткие фрагменты остаются в статье (min-lines по умолчанию 15).
 *
 * Usage:
 *   node scripts/migrate-parallel-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=11.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-16-parallelnye-vychisleniya',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'parallel-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-16-parallelnye-vychisleniya';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `code-416-${art}`;
  const seriesTitle = meta.title ?? `Параллельные вычисления ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-416-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['parallel-computing', 'hpc', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB parallel dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-06-cpp
 ...
 *
 * Usage:
 *   node scripts/migrate-cpp-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=21.md]
 */
...
const KB_DIR = path.join(KB_ROOT, 'docs', 'encyclopedia', '5-languages', '5-06-cpp');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'cpp-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-06-cpp';
...
function processArticle(filePath, relativePath) {
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `cpp-506-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB cpp dir not found: ${KB_DIR}`);
  }

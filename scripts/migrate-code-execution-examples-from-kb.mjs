/**
 * Извлекает листинги из docs/encyclopedia/4-code-dev/4-03-vypolnenie-koda
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Группирует короткие блоки одного раздела (сумма строк >= min-lines).
 * Русский псевдокод (text, plain, без языка) остаётся в статье.
 *
 * Usage:
 *   node scripts/migrate-code-execution-examples-from-kb.mjs [--dry-run] [--min-lines=8] [--only=114.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-03-vypolnenie-koda',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'code-execution-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-03-vypolnenie-koda';
...
function processArticle(filePath) {
  ...
  const seriesId = `code-403-${art}`;
  const seriesTitle = meta.title ?? `Выполнение кода ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-403-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['code-execution', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB code-execution dir not found: ${KB_DIR}`);
  }

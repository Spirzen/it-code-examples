/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-16-starye-yazyki/c-language
 ...
 *
 * Usage:
 *   node scripts/migrate-c-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=7.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'c-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-16-starye-yazyki/c-language';
...
function processArticle(filePath, relativePath) {
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `c-516-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB c-language dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает листинги из docs/encyclopedia/4-code-dev/4-06-arhitektura-vypolneniya
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Русский псевдокод (text, plain), mermaid и короткие CLI-фрагменты остаются в статье.
 *
 * Usage:
 *   node scripts/migrate-runtime-architecture-examples-from-kb.mjs [--dry-run] [--min-lines=8] [--only=113.md]
 */
...
const KB_DIR = path.join(
  ...
  'encyclopedia',
  '4-code-dev',
  '4-06-arhitektura-vypolneniya',
);
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'runtime-architecture-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-06-arhitektura-vypolneniya';

const args = process.argv.slice(2);
const DRY_RUN = args.includes('--dry-run');
const ONLY = args.find((a) => a.startsWith('--only='))?.slice('--only='.length);
const MIN_LINES = Number(args.find((a) => a.startsWith('--min-lines='))?.slice('--min-lines='.length) ?? 8);

const SKIP_FILES = new Set(['intro.md', '998.md', '999.md']);

const SKIP_FENCE_LANGS = new Set(['text', 'plain', 'plaintext', 'mermaid', 'scratch', 'regex', '']);
...
function processArticle(filePath) {
  ...
  const seriesId = `code-406-${art}`;
  const seriesTitle = meta.title ?? `Архитектура выполнения ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-406-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['runtime-architecture', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
  ...
  if (!DRY_RUN) {
    ...
    console.log(
      `[dry-run] update ${filename}: ${replacements.length} embed(s), ${replacements.reduce((n, r) => n + r.blocks.length, 0)} block(s)`,
    );
  }
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB runtime-architecture dir not found: ${KB_DIR}`);
  }

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/3-data-markup/3-07-sql
 * (it-knowledge-base) в examples/ и заменяет на ExternalCodeEmbed.
 *
 * Usage:
 *   node scripts/migrate-sql-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=55.md]
 */
...
const KB_SQL = path.join(KB_ROOT, 'docs', 'encyclopedia', '3-data-markup', '3-07-sql');
const EXAMPLES_ROOT = path.join(CODE_ROOT, 'examples');
const MANIFEST_PATH = path.join(__dirname, 'sql-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/3-data-markup/3-07-sql';
...
function processArticle(filePath) {
  ...
  const seriesId = `sql-${art}`;
  const seriesTitle = meta.title ?? `SQL ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `sql-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_SQL)) {
    throw new Error(`KB sql dir not found: ${KB_SQL}`);
  }
  ...
  for (const file of files) {
    const full = path.join(KB_SQL, file);

/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/5-languages/5-16-starye-yazyki/Cobol
 ...
 *
 * Usage:
 *   node scripts/migrate-cobol-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=7.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'cobol-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/5-languages/5-16-starye-yazyki/Cobol';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `cobol-516-${art}`;
  const seriesTitle = meta.title ?? `COBOL ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `cobol-516-${art}-${String(seriesOrder).padStart(3, '0')}`;
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB Cobol dir not found: ${KB_DIR}`);
  }

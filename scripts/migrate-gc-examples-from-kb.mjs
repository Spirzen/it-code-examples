/**
 * Извлекает тяжёлые листинги из docs/encyclopedia/4-code-dev/4-15-sborka-musora
 ...
 *
 * Usage:
 *   node scripts/migrate-gc-examples-from-kb.mjs [--dry-run] [--min-lines=15] [--only=1.md]
 */
...
const MANIFEST_PATH = path.join(__dirname, 'gc-examples-manifest.json');
const ENCYCLOPEDIA_BASE = '/encyclopedia/4-code-dev/4-15-sborka-musora';
...
function processArticle(filePath, relativePath) {
  ...
  const seriesId = `code-415-${art}`;
  const seriesTitle = meta.title ?? `Сборка мусора ${art}`;
  ...
  for (const group of groups) {
    seriesOrder += 1;
    const slugName = `code-415-${art}-${String(seriesOrder).padStart(3, '0')}`;
    ...
    writeExampleDir({
      ...
      meta: {
        ...
        tags: ['garbage-collection', 'encyclopedia', art],
        order: seriesOrder,
        series: seriesId,
        seriesOrder,
        seriesTitle,
...
function main() {
  if (!fs.existsSync(KB_DIR)) {
    throw new Error(`KB gc dir not found: ${KB_DIR}`);
  }
